...
| トークン | 値 | 用途 |
|---|---|---|
| `shiki-accent` | `#00a07e` | メインアクセント（ティール） |
| `shiki-bg` | `#f5f5f5` | ページ背景 |
| `shiki-card` | `#ffffff` | カード背景 |
| `shiki-surface` | `#f0f0f0` | インプット・サーフェス |

> ⚠️ `shiki-red` というトークン名は使わない。実際の色はティール（緑系）であり混乱を招くため。
...
> ⚠️ アフィリエイトIDは本番前に手動で設定する。**Claude Codeはアフィリエイト関連のIDを変更・削除しないこと。**
...
- **ユーザー認証は未実装。Claude Codeは認証機能を勝手に追加しないこと。** 認証が必要な機能は「未実装・今後の課題」に追記するにとどめる
